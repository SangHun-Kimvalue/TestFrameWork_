...
void NvEncoder::CreateDefaultEncoderParams(NV_ENC_INITIALIZE_PARAMS* pIntializeParams, GUID codecGuid, GUID presetGuid)
{
	...
	pIntializeParams->encodeConfig->gopLength = 30;// NVENC_INFINITE_GOPLENGTH;
		
	//pIntializeParams->encodeConfig->encodeCodecConfig.h264Config.sliceMode = 1;
	//pIntializeParams->encodeConfig->encodeCodecConfig.h264Config.sliceModeData = 1500 - 28;
    ...
    {
        pIntializeParams->encodeConfig->rcParams.constQP = { 26, 26, 36 };
    }

	/*pIntializeParams->encodeConfig->rcParams.enableMaxQP = 1;
	pIntializeParams->encodeConfig->rcParams.enableMinQP = 1;
	pIntializeParams->encodeConfig->rcParams.enableAQ = 1;
	pIntializeParams->encodeConfig->rcParams.targetQuality = 28;
	pIntializeParams->encodeConfig->rcParams.minQP = { 2, 2 , 25 };
	pIntializeParams->encodeConfig->rcParams.maxQP = { 51 , 51, 25 };
	pIntializeParams->encodeConfig->rcParams.enableInitialRCQP = 1;
	pIntializeParams->encodeConfig->rcParams.initialRCQP = { 20 };*/
	pIntializeParams->encodeConfig->rcParams.vbvBufferSize = 5000000;
    ...
    {
		pIntializeParams->encodeConfig->encodeCodecConfig.hevcConfig.repeatSPSPPS = 1;
...
void NvEncoder::GetSPSPPS(NV_ENC_SEQUENCE_PARAM_PAYLOAD* payload)
{
	NVENC_API_CALL(m_nvenc.nvEncGetSequenceParams(m_hEncoder, payload));
}
