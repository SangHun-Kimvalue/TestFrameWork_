...
void continueAfterDESCRIBE(RTSPClient* rtspClient, int resultCode, char* resultString) {
	do {
		...
		rtspClient->setupNextSubsession(rtspClient);
		return;
	} while (0);
...
void openURL(UsageEnvironment& env, char const* progName, char const* rtspURL) {
	...
	rtspClient->sendDescribeCommand((ourRTSPClient::continueAfterDESCRIBE));
...
typedef void(ourRTSPClient::*fptr)(void);

void ourRTSPClient::setupNextSubsession(RTSPClient* rtspClient) {
	UsageEnvironment& env = rtspClient->envir(); // alias
	StreamClientState& scs = ((ourRTSPClient*)rtspClient)->scs; // alias
	responseHandler* f = (responseHandler*)rtspClient;
	...
	if (scs.subsession != NULL) {
		if (!scs.subsession->initiate()) {
			...
			//fptr f = (void*)ourRTSPClient::continueAfterSETUP;

			//responseHandler
			
			continueAfterSETUP(rtspClient, 0, 0);
			// Continue setting up this subsession, by sending a RTSP "SETUP" command:
			rtspClient->sendSetupCommand(*scs.subsession, f, False, REQUEST_STREAMING_OVER_TCP);
			//rtspClient->sendSetupCommand(*scs.subsession, (responseHandler)continueAfterSETUP, False, REQUEST_STREAMING_OVER_TCP);
		}
		return;
	}
	...
	if (scs.session->absStartTime() != NULL) {
		...
		continueAfterPLAY(rtspClient, 0, 0);
		rtspClient->sendPlayCommand(*scs.session, f, scs.session->absStartTime(), scs.session->absEndTime());
	}
	else {
		scs.duration = scs.session->playEndTime() - scs.session->playStartTime();
		rtspClient->sendPlayCommand(*scs.session, f);
	}
}
