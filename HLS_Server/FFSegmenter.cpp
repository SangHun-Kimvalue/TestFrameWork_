...
FFSegmenter::FFSegmenter() : ISegmenter(0), Filename(""), UseAudio(false), Interval(5), VCo(AV_CODEC_ID_NONE), ACo(AV_CODEC_ID_NONE), DataQ(nullptr), Stopped(false) {



}

FFSegmenter::FFSegmenter(std::string Filename, QQ* DataQ, bool UseAudio, int Interval,
	AVCodecID VCo, AVCodecID ACo)
	:ISegmenter(0), UseAudio(UseAudio), Interval(Interval),
...
int FFSegmenter::write_frame(AVFormatContext *fmt_ctx, AVPacket* pkt)
{
	int ret = 0;
