...
{
	fUser = "User-DTrix.Converter : Simple RTSP Client\r\n";
	rtp = new RtpUnpacket();
	...
	do
	{
		...
		else if (resp.responseCode == 200)
		{
			...
			if (!resp.headers["Session"].empty())
			{
				...
				const char* ctemp = resp.headers["Session"].c_str();
				char* temp = strtok(const_cast<char*>(ctemp), ";");
				sessionID = temp;
...
unsigned RtspClient::WrapRTCPReport()
{
	...
	while (true)
	{
		...
		if (dwRet == WAIT_TIMEOUT)
		{
			if (sendVideoSetup)
			{
				auto s = std::string(""); //rtcp->PackRR(0);
				hd[1] = 0x01;
				hd[2] = (s.size() >> 8) & 0xFF;
				hd[3] = s.size() & 0xFF;
			...
			if (sendAudioSetup)
			{
				auto s = std::string(""); //rtcp->PackRR(1);
				hd[1] = 0x03;
				hd[2] = (s.size() >> 8) & 0xFF;
				hd[3] = s.size() & 0xFF;
				::send(m_cli, (char*)hd, 4, 0);
