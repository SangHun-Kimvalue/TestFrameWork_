...
int main(int argc, char **argv)
{
    ...
    strncpy_s(tmp_index, strlen(tmp_index),index, strlen(index) + 2);
    ...
    *dot = '.';
    ...
    if (url_fopen(&oc->pb, output_filename) < 0) {
    ...
    do {
        ...
        if (packet.stream_index == video_index /*&& (packet.flags & AV_PKT_FLAG_KEY)*/) {
            segment_time = (double)video_st->cur_dts * video_st->time_base.num / video_st->time_base.den;
        }
        else if (video_index < 0) {
            segment_time = (double)audio_st->cur_dts * audio_st->time_base.num / audio_st->time_base.den;
        }
        else {
