	...
	AVCodecContext **input_codec_context)
{
	...
	*input_format_context = avformat_alloc_context();
	...
	std::wstring wchar = L"audio=MicroPhone(Realtek High Definition Audio)";
	std::string device_name = "input_test.wav";
	int size = wchar.size();
	convert_utf8_string(device_name, wchar.c_str(), size);

	m_error = avformat_open_input(input_format_context, device_name.c_str(), iformat, NULL);
	//m_error = avformat_open_input(&ifmt_ctx, device_name.c_str(), NULL, NULL);
	if (m_error != 0) {
		error_pro(m_error, "Audio avformat_open_input Error");
		return m_error;
	}
...
int APIENTRY wWinMain(HINSTANCE hInstance,
	...
	std::string input = "input_test.mp3";
	std::string output = "transcode_out_test.avi";
	clock_t start = clock();
	clock_t end = 0;

	/* Open the input file for reading. */
	if (open_input_file(input.c_str(), &input_format_context,
		&input_codec_context))
		goto cleanup;
	/* Open the output file for writing. */
	if (open_output_file(output.c_str(), input_codec_context,
