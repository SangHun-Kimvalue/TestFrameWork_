// FFMPEG_Audio.cpp : Defines the entry point for the application.
//
...
//		codec_context = avcodec_alloc_context3(iformat_context->audio_codec);
//
	...
	int       nCmdShow)
{
	...
	const char* wfilename = "wtest.mp3";

	if (filename == "") {
		std::cout <<
			"Usage: " << filename << " input_file output_file"
			<< std::endl;
		return 1;
	}
	...
	std::vector<std::vector<double>> audio =
		audiorw::read(filename, sample_rate, 0, 10);

	// Write the file
	std::cout << "Writing to file " << wfilename << std::endl;
	audiorw::write(audio, wfilename, sample_rate);
...
//	double sample_rate = 44100;
//	std::vector<std::vector<double>> audio =
//		audiorw::read(filename, sample_rate);
//
//	// Write the file
//	std::cout << "Writing to file " << wfilename << std::endl;
//	audiorw::write(audio, wfilename, sample_rate);
