...
Connect_Handler::Connect_Handler(UsageEnvironment& env,
...
Connect_Handler* Connect_Handler::createNew(UsageEnvironment& env, Port ourPort,
	UserAuthenticationDatabase* authDatabase,
	unsigned reclamationSeconds) {
	int ourSocket = setUpOurSocket(env, ourPort);
	if (ourSocket == -1) return NULL;

	return new Connect_Handler(env, ourSocket, ourPort, authDatabase, reclamationSeconds);
}

char* Connect_Handler
...
char* Connect_Handler
::rtspURL(const char* serverMediaSession, int clientSocket) const {
	char* urlPrefix = rtspURLPrefix(clientSocket);
	char const* sessionName = serverMediaSession;
...
char* Connect_Handler::rtspURLPrefix(int clientSocket) const {
...
UserAuthenticationDatabase* Connect_Handler::setAuthenticationDatabase(UserAuthenticationDatabase* newDB) {
...
UserAuthenticationDatabase* Connect_Handler::getAuthenticationDatabaseForCommand(char const* /*cmdName*/) {
	// default implementation
	return fAuthDB;
}

Boolean Connect_Handler::specialClientAccessCheck(int /*clientSocket*/, struct sockaddr_in& /*clientAddr*/, char const* /*urlSuffix*/) {
	// default implementation
	return True;
}

Boolean Connect_Handler::specialClientUserAccessCheck(int /*clientSocket*/, struct sockaddr_in& /*clientAddr*/,
...
//void Connect_Handler::stopTCPStreamingOnSocket(int socketNum) {
...
//				= (Connect_Handler::RTSPClientSession*)lookupClientSession(sotcp->fSessionId);
...
char const* Connect_Handler::allowedCommandNames() {
	return "OPTIONS, DESCRIBE, SETUP, TEARDOWN, PLAY, PAUSE, GET_PARAMETER";
}

void Connect_Handler::RTSPClientConnection::resetRequestBuffer() {
...
void Connect_Handler::RTSPClientConnection::closeSocketsRTSP() {
...
void Connect_Handler::RTSPClientConnection::handleAlternativeRequestByte1(u_int8_t requestByte) {
	if (requestByte == 0xFF) {
...
void Connect_Handler::RTSPClientConnection
...
void Connect_Handler::RTSPClientConnection::handleAlternativeRequestByte(void* instance, u_int8_t requestByte) {
	RTSPClientConnection* connection = (RTSPClientConnection*)instance;
	connection->handleAlternativeRequestByte1(requestByte);
}

void Connect_Handler::RTSPClientConnection::handleRequestBytes(int newBytesRead) {
	int numBytesRemaining = 0;
	++fRecursionCount;

	do {
		Connect_Handler::RTSPClientSession* clientSession = NULL;
		...
		if (parseSucceeded) {
			...
			if (requestIncludedSessionId) {
				clientSession
					= (Connect_Handler::RTSPClientSession*)(fOurRTSPServer.lookupClientSession(sessionIdStr));
			...
			if (strcmp(cmdName, "OPTIONS") == 0) {
				...
				if (!requestIncludedSessionId) {
					...
					if (authenticationOK("SETUP", urlTotalSuffix, (char const*)fRequestBuffer)) {
						clientSession
							= (Connect_Handler::RTSPClientSession*)fOurRTSPServer.createNewClientSessionWithId();
					}
					else {
...
Boolean Connect_Handler::RTSPClientConnection::parseHTTPRequestString(char* resultCmdName, unsigned resultCmdNameMaxSize,
...
void Connect_Handler::RTSPClientConnection::handleCmd_bad() {
...
void Connect_Handler::RTSPClientConnection::handleCmd_notSupported() {
	snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
		"RTSP/1.0 405 Method Not Allowed\r\nCSeq: %s\r\n%sAllow: %s\r\n\r\n",
		fCurrentCSeq, dateHeader(), fOurRTSPServer.allowedCommandNames());
}

void Connect_Handler::RTSPClientConnection::handleCmd_notFound() {
	setRTSPResponse("404 Stream Not Found");
}

void Connect_Handler::RTSPClientConnection::handleCmd_sessionNotFound() {
	setRTSPResponse("454 Session Not Found");
}

void Connect_Handler::RTSPClientConnection::handleCmd_unsupportedTransport() {
	setRTSPResponse("461 Unsupported Transport");
}


void Connect_Handler::RTSPClientConnection::handleCmd_OPTIONS() {
	snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
		"RTSP/1.0 200 OK\r\nCSeq: %s\r\n%sPublic: %s\r\n\r\n",
		fCurrentCSeq, dateHeader(), fOurRTSPServer.allowedCommandNames());
}
...
Connect_Handler::createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr) {
	return new RTSPClientConnection(*this, clientSocket, clientAddr);
}

GenericMediaServer::ClientSession*
Connect_Handler::createNewClientSession(u_int32_t sessionId) {
	return new RTSPClientSession(*this, sessionId);
}

Boolean Connect_Handler::isRTSPServer() const {
	return True;
}

Connect_Handler::RTSPClientSession
...
Connect_Handler::RTSPClientSession::~RTSPClientSession() {
	reclaimStreamStates();
}

void Connect_Handler::RTSPClientSession::deleteStreamByTrack(unsigned trackNum) {
...
void Connect_Handler::RTSPClientSession::reclaimStreamStates() {
	for (unsigned i = 0; i < fNumStreamStates; ++i) {
		if (fStreamStates[i].subsession != NULL) {
...
void Connect_Handler::RTSPClientSession
::handleCmd_SETUP(Connect_Handler::RTSPClientConnection* ourClientConnection,
...
void Connect_Handler::RTSPClientSession
::handleCmd_withinSession(Connect_Handler::RTSPClientConnection* ourClientConnection,
	char const* cmdName,
...
void Connect_Handler::RTSPClientSession
::handleCmd_TEARDOWN(Connect_Handler::RTSPClientConnection* ourClientConnection,
	ServerMediaSubsession* subsession) {
	unsigned i;
...
void Connect_Handler::RTSPClientSession
::handleCmd_PLAY(Connect_Handler::RTSPClientConnection* ourClientConnection,
	...
	for (i = 0; i < fNumStreamStates; ++i) {
		if (subsession == NULL /* means: aggregated operation */
			...
			fStreamStates[i].subsession->startStream(fOurSessionId,
				...
				Connect_Handler::RTSPClientConnection::handleAlternativeRequestByte, ourClientConnection);
...
void Connect_Handler::RTSPClientSession
::handleCmd_PAUSE(Connect_Handler::RTSPClientConnection* ourClientConnection,
	ServerMediaSubsession* subsession) {
	for (unsigned i = 0; i < fNumStreamStates; ++i) {
		if (subsession == NULL /* means: aggregated operation */
			|| subsession == fStreamStates[i].subsession) {
...
void Connect_Handler::RTSPClientSession
::handleCmd_GET_PARAMETER(Connect_Handler::RTSPClientConnection* ourClientConnection,
	ServerMediaSubsession* /*subsession*/, char const* /*fullRequestStr*/) {
...
Boolean Connect_Handler::RTSPClientConnection
...
Connect_Handler::RTSPClientConnection
...
Connect_Handler::RTSPClientConnection::~RTSPClientConnection() {
