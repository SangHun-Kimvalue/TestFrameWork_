...
char* LiveRTSPServer::rtspURL(const char* serverMediaSession, int clientSocket) const {
	char* urlPrefix = rtspURLPrefix(clientSocket);
...
char* LiveRTSPServer::rtspURL(int clientSocket) const {
	char* urlPrefix = rtspURLPrefix(clientSocket);
...
char* LiveRTSPServer::rtspURLPrefix(int clientSocket) const {
...
void LiveRTSPServer::RTSPClientConnection::closeSocketsRTSP() {
...
void LiveRTSPServer::RTSPClientConnection::handleAlternativeRequestByte1(u_int8_t requestByte) {
	if (requestByte == 0xFF) {
...
Boolean LiveRTSPServer::RTSPClientConnection
...
void LiveRTSPServer::RTSPClientConnection::handleAlternativeRequestByte(void* instance, u_int8_t requestByte) {
	RTSPClientConnection* connection = (RTSPClientConnection*)instance;
	connection->handleAlternativeRequestByte1(requestByte);
}

void LiveRTSPServer::RTSPClientConnection::handleRequestBytes(int newBytesRead) {
	int numBytesRemaining = 0;
	++fRecursionCount;

	do {
		LiveRTSPServer::RTSPClientSession* clientSession = NULL;
...
//		}
		...
		if (parseSucceeded) {
			...
			if (requestIncludedSessionId) {
				clientSession
					= (LiveRTSPServer::RTSPClientSession*)(fOurRTSPServer.lookupClientSession(sessionIdStr));
			...
			if (strcmp(cmdName, "OPTIONS") == 0) {
				...
				if (!requestIncludedSessionId) {
					...
					if (authenticationOK("SETUP", urlTotalSuffix, (char const*)fRequestBuffer)) {
						clientSession
							= (LiveRTSPServer::RTSPClientSession*)fOurRTSPServer.createNewClientSessionWithId();
					}
					else {
...
Boolean LiveRTSPServer::RTSPClientConnection::parseHTTPRequestString(char* resultCmdName, unsigned resultCmdNameMaxSize,
...
void LiveRTSPServer::RTSPClientConnection::handleCmd_bad() {
...
void LiveRTSPServer::RTSPClientConnection::handleCmd_notSupported() {
	snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
		"RTSP/1.0 405 Method Not Allowed\r\nCSeq: %s\r\n%sAllow: %s\r\n\r\n",
		fCurrentCSeq, dateHeader(), fOurRTSPServer.allowedCommandNames());
}

void LiveRTSPServer::RTSPClientConnection::handleCmd_notFound() {
	setRTSPResponse("404 Stream Not Found");
}

void LiveRTSPServer::RTSPClientConnection::handleCmd_sessionNotFound() {
	setRTSPResponse("454 Session Not Found");
}

void LiveRTSPServer::RTSPClientConnection::handleCmd_unsupportedTransport() {
	setRTSPResponse("461 Unsupported Transport");
}


void LiveRTSPServer::RTSPClientConnection::handleCmd_OPTIONS() {
	snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
		"RTSP/1.0 200 OK\r\nCSeq: %s\r\n%sPublic: %s\r\n\r\n",
		fCurrentCSeq, dateHeader(), fOurRTSPServer.allowedCommandNames());
}
...
Boolean LiveRTSPServer::isRTSPServer() const {
	return True;
}

LiveRTSPServer::RTSPClientSession
...
LiveRTSPServer::RTSPClientSession::~RTSPClientSession() {
	reclaimStreamStates();
}

void LiveRTSPServer::RTSPClientSession::deleteStreamByTrack(unsigned trackNum) {
...
void LiveRTSPServer::RTSPClientSession::reclaimStreamStates() {
	for (unsigned i = 0; i < fNumStreamStates; ++i) {
		if (fStreamStates[i].subsession != NULL) {
...
::handleCmd_SETUP(LiveRTSPServer::RTSPClientConnection* ourClientConnection,
...
void LiveRTSPServer::RTSPClientSession
::handleCmd_withinSession(LiveRTSPServer::RTSPClientConnection* ourClientConnection,
	char const* cmdName,
...
void LiveRTSPServer::RTSPClientSession
::handleCmd_TEARDOWN(LiveRTSPServer::RTSPClientConnection* ourClientConnection,
	ServerMediaSubsession* subsession) {
	unsigned i;
...
void LiveRTSPServer::RTSPClientSession
::handleCmd_PLAY(LiveRTSPServer::RTSPClientConnection* ourClientConnection,
	...
	for (i = 0; i < fNumStreamStates; ++i) {
		if (subsession == NULL /* means: aggregated operation */
			...
			fStreamStates[i].subsession->startStream(fOurSessionId,
				...
				LiveRTSPServer::RTSPClientConnection::handleAlternativeRequestByte, ourClientConnection);
...
LiveRTSPServer::RTSPClientConnection::~RTSPClientConnection() {
